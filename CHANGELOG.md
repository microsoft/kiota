...
- TypeScript: a model property typed as a union with primitive collection members, such as `string[] | number[]`, was serialized as an object: the whole union went to `writeObjectValue`, and in a mixed union the `string[]` member was folded into `writeCollectionOfObjectValues`. Those members are now classified as primitives, guarded with `Array.isArray` and written with `writeCollectionOfPrimitiveValues`, matching how the composed type serializer already treats them. [#8018](https://github.com/microsoft/kiota/issues/8018)
