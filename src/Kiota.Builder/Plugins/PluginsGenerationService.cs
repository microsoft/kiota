...
#pragma warning restore CA2007 // Consider calling ConfigureAwait on the awaited task
    ...
    [LoggerMessage(Level = LogLevel.Warning, Message = "Skipping unsafe oauth_card_path file reference '{FilePath}' in the generated plugin manifest. An oauth_card_path file reference must be a relative path that does not point outside the manifest package (no absolute paths, URIs, or '..' parent-directory segments).")]
    private static partial void LogUnsafeOAuthCardPathFileReference(ILogger logger, string filePath);
