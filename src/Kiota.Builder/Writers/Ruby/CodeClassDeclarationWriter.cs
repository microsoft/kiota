    ...
    public class CodeClassDeclarationWriter : BaseElementWriter<CodeClass.Declaration, RubyConventionService>
    {
        ...
        public override void WriteCodeElement(CodeClass.Declaration codeElement, LanguageWriter writer)
        {
            foreach (var codeUsing in codeElement.Usings
                                        ...
                                        .OrderBy(x => x.Key))
            {
                //TODO: resolve internal dependencies importing
                writer.WriteLine($"require_relative '{codeUsing.Key}'");
            }
            foreach (var codeUsing in codeElement.Usings
                                        ...
                                        .OrderBy(x => x.Key))
            {
                // TODO: resolve external dependencies importing             
                writer.WriteLine($"require '{codeUsing.Select(x => x.importSymbol).Distinct().Aggregate((x,y) => x + ", " + y)}'");
            }
            writer.WriteLine();
