...
public class CodeConstantWriter : BaseElementWriter<CodeConstant, TypeScriptConventionService>
{
    ...
    private void WriteRequestsMetadataConstant(CodeConstant codeElement, LanguageWriter writer)
    {
        ...
        if (codeClass.Methods
            ...
            return;
        var uriTemplateConstant = codeElement.Parent is CodeFile parentFile && parentFile.Constants.FirstOrDefault(x => x.Kind is CodeConstantKind.UriTemplate && x.OriginalCodeElement == codeClass) is CodeConstant tplct ?
