...
public abstract class CodeComposedTypeBase : CodeTypeBase, IDiscriminatorInformationHolder, IDeprecableElement
{
    ...
    public IEnumerable<CodeType> Types
    {
        get
        {
            var orderedTypes = new List<CodeType>(types.Values);
            orderedTypes.Sort((x, y) => string.Compare(NormalizeKey(x), NormalizeKey(y), StringComparison.OrdinalIgnoreCase));
            return orderedTypes;
        }
    }
    ...
    protected override TChildType BaseClone<TChildType>(CodeTypeBase source, bool cloneName = true)
    {
        ...
        if (!(source is CodeComposedTypeBase sourceComposed))
        {
        ...
        if (sourceComposed.Types != null)
        {
            foreach (var type in sourceComposed.Types)
            {
                AddType(type);
            }
        }
