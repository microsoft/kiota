...
internal static class KiotaConfigurationExtensions
{
    ...
    public static void BindConfiguration(this KiotaConfiguration configObject, IConfigurationRoot configuration)
    {
        ...
        configObject.Search.Apicurio.ApiBaseUrl = Uri.TryCreate(configuration[$"{nameof(configObject.Search)}:{nameof(SearchConfiguration.Apicurio)}:{nameof(ApicurioConfiguration.ApiBaseUrl)}"], new UriCreationOptions(), out var apicurioApiBaseUrl) ? apicurioApiBaseUrl : configObject.Search.Apicurio.ApiBaseUrl;
        configObject.Search.Apicurio.UIBaseUrl = Uri.TryCreate(configuration[$"{nameof(configObject.Search)}:{nameof(SearchConfiguration.Apicurio)}:{nameof(ApicurioConfiguration.UIBaseUrl)}"], new UriCreationOptions(), out var uiBaseUrl) ? uiBaseUrl : configObject.Search.Apicurio.UIBaseUrl;
        configObject.Search.Apicurio.ArtifactsLimit = configuration[$"{nameof(configObject.Search)}:{nameof(SearchConfiguration.Apicurio)}"] is string artifactsLimit && !string.IsNullOrEmpty(artifactsLimit) ? int.Parse(artifactsLimit) : configObject.Search.Apicurio.ArtifactsLimit;
        configObject.Search.Apicurio.VersionsLimit = configuration[$"{nameof(configObject.Search)}:{nameof(SearchConfiguration.Apicurio)}"] is string versionsLimit && !string.IsNullOrEmpty(versionsLimit) ? int.Parse(versionsLimit) : configObject.Search.Apicurio.VersionsLimit;
        configObject.Search.Apicurio.SearchBy = configuration[$"{nameof(configObject.Search)}:{nameof(SearchConfiguration.Apicurio)}"] is string searchBy && !string.IsNullOrEmpty(searchBy) ? (ApicurioConfiguration.ApicurioSearchBy)Enum.Parse(typeof(ApicurioConfiguration.ApicurioSearchBy), searchBy) : configObject.Search.Apicurio.SearchBy;
