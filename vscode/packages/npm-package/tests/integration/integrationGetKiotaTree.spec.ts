...
function findOperationByPath(
  actual: KiotaTreeResult | undefined,
  path: string,
): KiotaOpenApiNode | undefined {
  if (!actual?.rootNode) return undefined;
  const flattenedTree = flattenKiotaTree(actual.rootNode);
  const operation = flattenedTree.find(
    (child) => child.isOperation === true && child.path === path,
  );
  return operation;
}
...
describe("getKiotaTree", () => {
  test("testGetKiotaTree_from_valid_File", async () => {
    ...
    const actual = await getKiotaTree({
      includeFilters: [],
    ...
    expect(
      existsEqualOrGreaterThanLevelLogs(actual?.logs, LogLevel.warning),
    ).toBeFalsy();
    expect(
      existsEqualOrGreaterThanLevelLogs(actual?.logs, LogLevel.information),
    ).toBeTruthy();
    const actualOperationNode = findOperationByPath(
      actual,
      "\\discriminateme#POST",
    );
  ...
  test("testGetKiotaTree_withSecurity", async () => {
    const descriptionUrl =
      "../../../tests/Kiota.Builder.IntegrationTests/ModelWithSecurity.yaml";
    ...
    const actualListOperationNode = findOperationByPath(
      actual,
      "\\repairs#GET",
    );
    ...
    const firstSecurityRequirementInGet =
      actualListOperationNode?.security?.[0];
    const oAuth2AuthCodeSecurityRequirementInGet =
      firstSecurityRequirementInGet?.["oAuth2AuthCode"];
    expect(oAuth2AuthCodeSecurityRequirementInGet).toBeDefined();
    expect(oAuth2AuthCodeSecurityRequirementInGet?.length).toEqual(1);
    expect(oAuth2AuthCodeSecurityRequirementInGet?.[0]).toEqual(
      "api://sample/repairs_read",
    );
    ...
    const actualPostOperationNode = findOperationByPath(
      actual,
      "\\repairs#POST",
    );
    ...
    const firstSecurityRequirementInPost =
      actualPostOperationNode?.security?.[0];
    const oAuth2AuthCodeSecurityRequirementInPost =
      firstSecurityRequirementInPost?.["oAuth2AuthCode"];
    ...
    expect(oAuth2AuthCodeSecurityRequirementInPost?.[0]).toEqual(
      "api://sample/repairs_write",
    );

    // Check if the security schemes are defined correctly for post operation
    const actualOAuthSecuritySchema = actual?.securitySchemes?.[
      "oAuth2AuthCode"
    ] as OAuth2SecurityScheme;
    expect(actualOAuthSecuritySchema).toBeDefined();
    expect(actualOAuthSecuritySchema.flows).toBeDefined();
    const actualAuthorizationCodeFlow =
      actualOAuthSecuritySchema.flows.authorizationCode;
    expect(actualAuthorizationCodeFlow).toBeDefined();
    expect(actualAuthorizationCodeFlow?.authorizationUrl).toEqual(
      "https://login.microsoftonline.com/common/oauth2/v2.0/authorize",
    );
    expect(actualAuthorizationCodeFlow?.tokenUrl).toEqual(
      "https://login.microsoftonline.com/common/oauth2/v2.0/token",
    );
    expect(actualAuthorizationCodeFlow?.refreshUrl).toEqual(
      "https://login.microsoftonline.com/common/oauth2/v2.0/refresh",
    );
    expect(actualAuthorizationCodeFlow?.scopes).toBeDefined();
    expect(
      actualAuthorizationCodeFlow?.scopes["api://sample/repairs_read"],
    ).toEqual("Read repair records");
    expect(
      actualAuthorizationCodeFlow?.scopes["api://sample/repairs_write"],
    ).toEqual("Write repair records");
    const actualImplicitFlow = actualOAuthSecuritySchema.flows.implicit;
    expect(actualImplicitFlow).toBeDefined();
    expect(actualImplicitFlow?.authorizationUrl).toEqual(
      "https://login.microsoftonline.com/common/oauth2/v2.0/authorize",
    );
    expect(actualImplicitFlow?.refreshUrl).toEqual(
      "https://login.microsoftonline.com/common/oauth2/v2.0/refresh",
    );
    expect(actualImplicitFlow?.scopes).toBeDefined();
    expect(actualImplicitFlow?.scopes["api://sample/repairs_read"]).toEqual(
      "Read repair records",
    );
    expect(actualImplicitFlow?.scopes["api://sample/repairs_write"]).toEqual(
      "Write repair records",
    );
    const actualClientCredentialsFlow =
      actualOAuthSecuritySchema.flows.clientCredentials;
    expect(actualClientCredentialsFlow).toBeDefined();
    expect(actualClientCredentialsFlow?.tokenUrl).toEqual(
      "https://login.microsoftonline.com/common/oauth2/v2.0/token",
    );
    expect(actualClientCredentialsFlow?.refreshUrl).toEqual(
      "https://login.microsoftonline.com/common/oauth2/v2.0/refresh",
    );
    expect(actualClientCredentialsFlow?.scopes).toBeDefined();
    expect(
      actualClientCredentialsFlow?.scopes["api://sample/repairs_read"],
    ).toEqual("Read repair records");
    expect(
      actualClientCredentialsFlow?.scopes["api://sample/repairs_write"],
    ).toEqual("Write repair records");
    const actualPasswordFlow = actualOAuthSecuritySchema.flows.password;
    expect(actualPasswordFlow).toBeDefined();
    expect(actualPasswordFlow?.tokenUrl).toEqual(
      "https://login.microsoftonline.com/common/oauth2/v2.0/token",
    );
    expect(actualPasswordFlow?.refreshUrl).toEqual(
      "https://login.microsoftonline.com/common/oauth2/v2.0/refresh",
    );
    expect(actualPasswordFlow?.scopes).toBeDefined();
    expect(actualPasswordFlow?.scopes["api://sample/repairs_read"]).toEqual(
      "Read repair records",
    );
    expect(actualPasswordFlow?.scopes["api://sample/repairs_write"]).toEqual(
      "Write repair records",
    );

    const actualHttpSecuritySchema = actual?.securitySchemes?.[
      "httpAuth"
    ...
    expect(actualHttpSecuritySchema.description).toEqual(
      "HTTP basic authentication",
    );

    const actualApiKeySecuritySchema = actual?.securitySchemes?.[
      "apiKeyAuth"
    ...
    expect(actualApiKeySecuritySchema.description).toEqual(
      "API key authentication",
    );

    const actualOpenIdConnectSecuritySchema = actual?.securitySchemes?.[
      "openIdConnectAuth"
    ] as OpenIdSecurityScheme;
    expect(actualOpenIdConnectSecuritySchema).toBeDefined();
    expect(actualOpenIdConnectSecuritySchema.type).toEqual("openIdConnect");
    expect(actualOpenIdConnectSecuritySchema.description).toEqual(
      "OpenID Connect authentication",
    );
    expect(actualOpenIdConnectSecuritySchema.openIdConnectUrl).toEqual(
  ...
  test("testGetKiotaTree_withMultipleSecurity", async () => {
    ...
    const secondSecurityRequirementInPost =
      actualPostOperationNode?.security?.[1];
    expect(secondSecurityRequirementInPost).toBeDefined();
    const httpAuthSecurityRequirementInPost =
      secondSecurityRequirementInPost?.["httpAuth"];
    ...
    const oAuth2AuthCodeSecurityRequirementInPut =
      firstSecurityRequirementInPut?.["oAuth2AuthCode"];
    expect(oAuth2AuthCodeSecurityRequirementInPut).toBeDefined();
    expect(oAuth2AuthCodeSecurityRequirementInPut?.length).toEqual(1);
    expect(oAuth2AuthCodeSecurityRequirementInPut?.[0]).toEqual(
      "api://sample/repairs_write",
    );
    const httpAuthSecurityRequirementInPut =
      firstSecurityRequirementInPut?.["httpAuth"];
  ...
  test("testGetKiotaTree_withMultipleSecurityAndVariables", async () => {
    const descriptionUrl =
      "../../../tests/Kiota.Builder.IntegrationTests/ModelWithMultipleSecurityAndVariables.yaml";
    ...
    expect(actualAuthorizationCodeFlow?.tokenUrl).toEqual(
      "https://login.microsoftonline.com/${{AAD_APP_TENANT_ID}}/oauth2/v2.0/token",
    );
    expect(actualAuthorizationCodeFlow?.refreshUrl).toEqual(
      "https://login.microsoftonline.com/${{AAD_APP_TENANT_ID}}/oauth2/v2.0/refresh",
    );
    expect(actualAuthorizationCodeFlow?.scopes).toBeDefined();
    expect(
      actualAuthorizationCodeFlow?.scopes["api://sample/repairs_read"],
    ...
    expect(actualOpenIdConnectSecuritySchema.openIdConnectUrl).toEqual(
      "https://login.microsoftonline.com/${{AAD_APP_TENANT_ID}}/.well-known/openid-configuration",
    );
  });

  test("testGetKiotaTree_withReferenceIdExtension", async () => {
    const descriptionUrl =
      "../../../tests/Kiota.Builder.IntegrationTests/ModelWithRefIdExtension.yaml";
    const actual = await getKiotaTree({
      includeFilters: [],
      descriptionPath: descriptionUrl,
      excludeFilters: [],
    ...
    expect(actualSecuritySchema?.description).toEqual(
      "OAuth configuration for the repair service",
    );
    expect(actualSecuritySchema?.referenceId).toEqual("otherValue123");

    const securitySchemeAsOauthScheme =
      actualSecuritySchema as OAuth2SecurityScheme;
    ...
    expect(
      securitySchemeAsOauthScheme.flows.authorizationCode?.authorizationUrl,
    ).toEqual("https://login.microsoftonline.com/common/oauth2/v2.0/authorize");
    expect(
      securitySchemeAsOauthScheme.flows.authorizationCode?.tokenUrl,
    ).toEqual("https://login.microsoftonline.com/common/oauth2/v2.0/token");
    expect(
      securitySchemeAsOauthScheme.flows.authorizationCode?.scopes,
    ).toBeDefined();
    expect(
      securitySchemeAsOauthScheme.flows.authorizationCode?.scopes[
        "api://sample/repairs_read"
  ...
  test("testGetKiotaTree_withoutBasicInfoInOneOperation", async () => {
    const descriptionUrl =
      "../../../tests/Kiota.Builder.IntegrationTests/ModelWithoutBasicInfoInOneOperation.yaml";
    const actual = await getKiotaTree({
      includeFilters: [],
      descriptionPath: descriptionUrl,
      excludeFilters: [],
    ...
    expect(actualListOperationNode).toBeDefined();
    expect(actualListOperationNode?.operationId).toEqual("listRepairs");
    expect(actualListOperationNode?.summary).toEqual(
      "List all repairs with oauth",
    );
    expect(actualListOperationNode?.description).toEqual(
      "Returns a list of repairs with their details and images",
    );
    expect(actualListOperationNode?.isOperation).toBeTruthy();
    expect(actualListOperationNode?.documentationUrl).toEqual(
      "https://sample.server/api/docs",
    );
  ...
  test("testGetKiotaTree_withOverriddenServer", async () => {
    const descriptionUrl =
      "../../../tests/Kiota.Builder.IntegrationTests/ModelWithOverriddenServer.yaml";
    ...
    expect(actualOperationNode?.servers?.[0]).toEqual(
      "https://sample.server.overridden/api",
    );
  });

  test("testGetKiotaTree_from_valid_external", async () => {
    const descriptionUrl =
      "https://raw.githubusercontent.com/microsoftgraph/msgraph-sdk-powershell/refs/heads/main/openApiDocs/v1.0/Mail.yml";
  ...
  test("testGetKiotaTree_withAdaptiveCard", async () => {
    const descriptionUrl =
      "../../../tests/Kiota.Builder.IntegrationTests/ModelWithAdaptiveCardExtension.yaml";
    ...
    expect(actualOperationNode?.adaptiveCard?.file).toEqual(
      "path_to_adaptive_card_file",
    );
  ...
  test("testGetKiotaTree_withNoServer_withKiotaValidationRules", async () => {
    const descriptionUrl =
      "../../../tests/Kiota.Builder.IntegrationTests/ModelWithNoServer.yml";
    const actual = await getKiotaTree({
      includeFilters: [],
      descriptionPath: descriptionUrl,
      excludeFilters: [],
    ...
    expect(actual?.rootNode?.children[0].children[0].operationId).toEqual(
      "listRepairs",
    );

    // Maximum log level is warning, so we should not have any logs with level greater than warning
    const actualLogsGreaterThanWarning = actual?.logs.filter(
    ...
    const actualLogNoServerEntry = actual?.logs?.find((log) =>
      log.message.startsWith(
        "OpenAPI warning: #/ - A servers entry (v3) or host + basePath + schemes properties (v2) was not present in the OpenAPI description.",
      ),
    );
  ...
  test("testGetKiotaTree_fromV2_0", async () => {
    const descriptionUrl =
      "../../../tests/Kiota.Builder.IntegrationTests/SwaggerPetStore.json";
  ...
  test("testGetKiotaTree_fromV3_1", async () => {
    const descriptionUrl =
      "../../../tests/Kiota.Builder.IntegrationTests/SimpleModelOpenApi3_1.yaml";
